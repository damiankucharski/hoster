...
window.FarmGod.Main = (function (Library, Translation) {
  ...
  const bindEventHandlers = function () {
      ...
      // Add auto-send button handler
      $('.autoSendButton').off('click').on('click', function() {
          let $button = $(this);
          $button.prop('disabled', true);
          $button.val('Auto-sending in progress...');
          
          // Start the auto-sending process
          autoSendFarms();
      });
  };
  ...
  const autoSendFarms = function() {
      if (autoSendInProgress) return;
      
      autoSendInProgress = true;
      
      // Get all available farm icons and add them to the queue
      autoSendQueue = $('.farmGod_icon').toArray().map(el => $(el));
      
      // Process the queue
      processAutoSendQueue();
  };
  
  const processAutoSendQueue = function() {
      if (autoSendQueue.length === 0) {
          // Queue is empty, we're done
          autoSendInProgress = false;
          $('.autoSendButton').val('All farms sent!').prop('disabled', true);
          return;
      }
      
      // Get the next farm to send
      let $farm = autoSendQueue.shift();
      
      // Check if the farm is valid for sending
      if (game_data.market != 'nl' || $farm.data('origin') == curVillage) {
          sendFarm($farm);
      } else {
          // Skip this farm and continue with the next one
          setTimeout(processAutoSendQueue, 50);
          return;
      }
      
      // Update the button text to show progress
      let remaining = autoSendQueue.length;
      $('.autoSendButton').val(`Auto-sending... (${remaining} remaining)`);
      
      // Wait for the specified delay before sending the next farm
      setTimeout(processAutoSendQueue, 250);
  };
  ...
  const buildTable = function (plan) {
    let html = `<div class="vis farmGodContent"><h4>FarmGod</h4><table class="vis" width="100%">
       ...
       // Add the auto-send button here
       html += `<tr>
                 <td colspan="4" style="text-align: center;">
                   <input type="button" class="btn autoSendButton" value="Auto-Send All Farms (0.25s delay)" style="margin: 5px;">
                 </td>
               </tr>`;
  ...
  const sendFarm = function ($this) {
      ...
      if (!farmBusy && !(Accountmanager.farm.last_click && n - Accountmanager.farm.last_click < 200)) {
          ...
          // If we're busy, wait a bit and try again (only for auto-send)
          if (autoSendInProgress) {
              setTimeout(() => {
                  // Push this farm back to the front of the queue
                  autoSendQueue.unshift($this);
                  processAutoSendQueue();
              }, 500);
          }
      }
